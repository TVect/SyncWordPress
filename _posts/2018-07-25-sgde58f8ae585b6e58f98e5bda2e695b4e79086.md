...
Momentum 更新公式为: $$\\theta_{t+1} = \\theta_{t} - (\\gamma m_{t-1} + \\eta g_{t})$$
另外，可以对 NAG 的更新公式做一些修改， 直接使用 look-ahead momentum vector 来更新当前参数，而不是先用 moment更新梯度，再使用moment更新参数值。
$$
\\begin{aligned}
g_{t} &= \\bigtriangledown_{\\theta_{t}}J(\\theta_{t}) \\\\
\\\\
...
Adam 的更新公式为：
$$
\\theta_{t+1} = \\theta_{t} - \\frac {\\eta}{v_{t}} \\hat{m_{t}} = \\theta_{t} - \\frac {\\eta}{v_{t}} (\\beta_{1}\\hat{m_{t-1}} + \\frac{1-\\beta_{1}}{1-\\beta_{1}^{t}}g_{t})
$$
为了可以在计算梯度时使用了未来位置的信息，这里替换 $$\\hat{m_{t-1}}$$ 为 $$\\hat{m_{t}}$$, 得到 Nadam 的更新规则为：
$$
\\theta_{t+1} = \\theta_{t} - \\frac {\\eta}{v_{t}} (\\beta_{1}\\hat{m_{t}} + \\frac{1-\\beta_{1}}{1-\\beta_{1}^{t}}g_{t})
$$</p></li>
</ul>
