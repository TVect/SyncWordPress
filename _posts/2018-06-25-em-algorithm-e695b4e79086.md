...
&= \\sum \\limits_{j=1}^{n} \\sum \\limits_{l=1}^{k} p(z_{j} | x_{j}, \\theta^{old}) ln \\, \\alpha_{l} N(x_{i} | u_{l}, \\Sigma_{l}) \\\\
&= \\sum \\limits_{j=1}^{n} \\sum \\limits_{l=1}^{k} p(z_{j} | x_{j}, \\theta^{old}) (ln \\, \\alpha_{l} - \\frac{D}{2} ln\\,2\\pi - \\frac{1}{2}ln\\,|\\Sigma| + (x_{i}-u_{l})^{T}\\Sigma^{-1}(x_{i}-u_{l}))
\\end{aligned}
$$</li>
</ol>

分别关于参数 $$\\alpha, u, \\Sigma$$ 做优化有：

<ul>
<li>关于 $$\\alpha$$ 做优化
优化时涉及到的相关项为: $$A = \\sum \\limits_{j=1}^{n} \\sum \\limits_{l=1}^{k} p(z_{j} | x_{j}, \\theta^{old}) ln \\, \\alpha_{l}$$.</li>
</ul>

同时考虑到约束条件$$\\sum_{l=1}^{k}\\alpha_{l}=1$$, 使用 Lagrange Multiplier, 有：
$$
\\begin{aligned}
LM(\\alpha, \\lambda) &= \\sum \\limits_{j=1}^{n} \\sum \\limits_{l=1}^{k} p(z_{j} | x_{j}, \\theta^{old}) ln \\, \\alpha_{l} - \\lambda (\\sum_{l=1}^{k}\\alpha_{l} - 1) \\\\
